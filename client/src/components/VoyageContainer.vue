...
<template>
  ...
  <section v-else class="flex flex-col gap-10 p-4">
    <Transition name="slide-in">
      <BaseModal
        v-show="isModalShowing"
        :show="isModalShowing"
        :title="selectedVoyage?.name"
        :open="isModalShowing"
        @close="isModalShowing = false"
        uppercase
        titleClass="font-sci-fi"
      >
    ...
    <TransitionGroup name="fade" tag="div" appear class="flex flex-wrap gap-4">
      <VoyageCard
        v-for="(voyage, index) in props.voyages"
        :key="voyage.id"
        :voyage="voyage"
        @join-voyage="emits('join-voyage', voyage)"
